<template>
  <div class="setting-batch-action">
    <div class="item" v-for="(item, batchIndex) in batchActionSetting" :key="batchIndex">
      <el-form
        ref="form"
        :model="item"
        label-width="100px"
      ...
      >
        ...
        <el-form-item>
          <el-button-group>
            ...
            <el-button :disabled="batchIndex === batchActionSetting.length - 1" @click="moveAction(batchIndex)">下移</el-button>
...
export default {
  ...
  data() {
    return {
      batchActionSetting: [],  // 批量操作权限列表
    }
  },
  methods: {
    // 移动批量操作
    moveAction(index, direction) {
      let item = this.batchActionSetting[index],         // 需要移动的元素
        targetIndex = direction ? index - 1 : index + 1  // 目标下标
      this.batchActionSetting.splice(index, 1)
      this.batchActionSetting.splice(targetIndex, 0, item)
    },
    // 删除批量操作
    deleteAction(index) {
    ...
    // 增加权限
    addOperation() {
      this.batchActionSetting.push({ buttonText: '', operation: '' })
    }
  }
}
</script>
