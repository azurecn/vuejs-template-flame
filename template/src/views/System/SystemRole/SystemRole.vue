...
export default {
  ...
  methods: {
    ...
    openRouterAccess({ scope }) {
      this.roleId = scope.row.flame_id
      callPost({ tableName: 'sysflamemenu', data: { role_id: scope.row.flame_id }}, 'GetRoleMenuTree')
        .then(({ item }) => {
    ...
    submitRouterAccess() {
      const submitData = {
        tableName: 'SysFlameRoleShipMenu',
        data: {
          role_id: this.roleId,
    ...
    openPageAccess({ scope }) {
      this.roleId = scope.row.flame_id
      callPost({ tableName: 'sysflamemenu', data: { role_id: scope.row.flame_id }}, 'GetRoleCheckMenuTree')
        .then(({ item }) => {
    ...
    changeRoleRoute(event) {
      ...
      const submitData = {
        ...
        data: { menu_flame_id: this.menuId, role_flame_id: this.roleId }
      }
      callPost(submitData, 'GetPageConfigByRoleAndMenuid')
        .then(({ item }) => {
    ...
    submitPageAccess() {
      ...
      const submitData = {
        ...
        data: {
          ...
          role_flame_id: this.roleId,
          data: this.pageSettingBackup
        }
      }
      callPost(submitData, 'AddRolePageConfig')
